...
def get_request():
    ...
    if ask == 'd':
        in_filename = tkinter.filedialog.askopenfilename(filetypes=files, defaultextension=files)
        out_filename = tkinter.filedialog.asksaveasfilename(filetypes=files, defaultextension=files)
        decompress(in_filename, out_filename)  # 解压文件
    elif ask == 'c':
        in_filename = tkinter.filedialog.askopenfilename(filetypes=files, defaultextension=files)
        out_filename = tkinter.filedialog.asksaveasfilename(filetypes=files, defaultextension=files)
        compress(in_filename, out_filename)  # 压缩文件
    else:
